...
When a remote declares `exposes`, the plugin generates an instance-scoped virtual module (for example `virtual:mf-exposes:<scope>`) containing a map of expose names to async import functions. Generated by `generateExposes(options)` in `src/virtualModules/virtualExposes.ts`:
...
```js
import exposesMap from 'virtual:mf-exposes:<scope>';
...
When a host app requests the remote's `remoteEntry.js` URL, the middleware redirects the request to that federation instance's virtual module path (for example `@id/virtual:mf-REMOTE_ENTRY_ID:<scope>`), which Vite's dev server then resolves and serves through the normal plugin pipeline.

The `pluginProxyRemoteEntry` plugin handles the actual code generation via its `resolveId`/`load`/`transform` hooks. Each federation() instance passes its own `options`, `remoteEntryId`, and `virtualExposesId` into the plugin, which prevents ID/config collisions when multiple federation instances are used in one Vite config. In dev mode `parsePromise` resolves immediately (no module-parse tracking), so the code is generated synchronously.
