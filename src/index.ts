...
/**
 * The automatic JSX runtime import a dev transform injects. It is generated by
 * the compiler and never written in user source, so a textual crawl of the
 * entry graph cannot see it - it has to be recorded explicitly.
 */
const DEV_AUTOMATIC_JSX_RUNTIME = 'react/jsx-dev-runtime';
...
function registerEntryImports(
  ...
  while (pending.length) {
    ...
    // A JSX file imports the automatic runtime even though no import statement
    // for it exists in the source, so record it before the textual scan below.
    // Without this the share is only ever observed by the resolver hook this
    // plugin installs into optimizeDeps, which Vite runs only while it scans
    // dependencies - and it skips scanning entirely once node_modules/.vite is
    // warm. The share then stays unmaterialized, init() never seeds it, and its
    // loadShare wrapper hands the entry an undefined binding.
    if (
      recordShared &&
      JSX_SOURCE_EXTENSIONS.some((extension) => file.endsWith(extension)) &&
      findSharedKey(DEV_AUTOMATIC_JSX_RUNTIME, options.shared)
    ) {
      addUsedShares(DEV_AUTOMATIC_JSX_RUNTIME, options);
    }
