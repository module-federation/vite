...
describe('vite:module-federation-early-init', () => {
  ...
  it.each([
    { reactScope: 'default', materialized: true },
    { reactScope: 'heavy-ui', materialized: false },
  ])(
    'only materializes package dependencies from the same share scope',
    ({ reactScope, materialized }) => {
      const root = mkdtempSync(path.join(tmpdir(), 'mf-entry-share-scope-'));
      mkdirSync(path.join(root, 'src'));
      writeFileSync(
        path.join(root, 'index.html'),
        '<script type="module" src="/src/main.ts"></script>'
      );
      writeFileSync(path.join(root, 'src/main.ts'), 'import "react-dom"');

      try {
        const plugins = federation({
          name: 'entry-share-scope',
          shared: {
            react: { shareScope: reactScope },
        ...
        const code = generateLocalSharedImportMap(owner._options);
        expect(code).toMatch(/"react-dom": \{[\s\S]*?materialize: true,/);
        expect(code).toMatch(new RegExp(`"react": \\{[\\s\\S]*?materialize: ${materialized},`));
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
    }
